...
ENABLED=$(tmux show-option -gv @city-blue_show_battery_widget 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
BATTERY_NAME=$(tmux show-option -gv @city-blue_battery_name 2>/dev/null)
BATTERY_LOW=$(tmux show-option -gv @city-blue_battery_low_threshold 2>/dev/null)
RESET="#[fg=brightwhite,bg=#0a1929,nobold,noitalics,nounderscore,nodim]"
...
echo "#[fg=$ICON_COLOR,bg=$PILL,bold] $ICON #[fg=$FG,bg=$PILL,nobold] ${BATTERY_PERCENTAGE}% "
