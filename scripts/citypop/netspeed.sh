...
# Check if enabled
ENABLED=$(tmux show-option -gv @city-blue_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0

# Imports
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${ROOT_DIR}/lib/palette.sh"
source "${ROOT_DIR}/lib/netspeed-core.sh"

# Get network interface
INTERFACE=$(tmux show-option -gv @city-blue_netspeed_iface 2>/dev/null)
# Show IP address
SHOW_IP=$(tmux show-option -gv @city-blue_netspeed_showip 2>/dev/null)
# Time between refresh
TIME_DIFF=$(tmux show-option -gv @city-blue_netspeed_refresh 2>/dev/null)
...
if [[ -z $INTERFACE ]]; then
  ...
  tmux set-option -g @city-blue_netspeed_iface "$INTERFACE"
fi
...
NETWORK_ICON_RAW=${RAW_ICONS[${IFACE_TYPE}_${IFACE_STATUS}]}

BG_BAR="${THEME[background]}"
PILL_BG="${THEME[bblack]}"
FG_C="${THEME[foreground]}"
ACCENT_NET="${THEME[blue]}"
[[ ${IFACE_STATUS} == "down" ]] && ACCENT_NET="${THEME[red]}"
OUTPUT="#[fg=$ACCENT_NET,bg=$PILL_BG,bold] $NETWORK_ICON_RAW #[fg=$FG_C,bg=$PILL_BG,nobold] $INTERFACE "
